...
void loop() {
  ...
  if(digitalRead(sw) == LOW) { //스위치를 눌렀다면
    digitalWrite(greenLED, LOW); //초록색 불끄기
    digitalWrite(redLED, HIGH); //빨간색LED 불 켜기
  } else {                      // 스위치에서 손을 떼었다면
    digitalWrite(redLED, LOW); //빨간색LED 불끄기
    digitalWrite(greenLED, HIGH); //초록색LED 불 켜기
  }
